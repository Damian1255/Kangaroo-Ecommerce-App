...
class CollectionVewController: UIViewController, UIScrollViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    
    let discoveryIdentifier = "collectionCell"
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let collectionCell = collectionView.dequeueReusableCell(withReuseIdentifier: discoveryIdentifier, for: indexPath) as! collectionCell
        
        collectionCell.collectionImage.image = UIImage(named: ProductImage[indexPath.row])
        collectionCell.collectionName.text = ProductName[indexPath.row]
        collectionCell.collectionRating.rating = ProductRating[indexPath.row]
        collectionCell.collectionRating.text = "(\(Int(ProductRating[indexPath.row])))"
         ...
         if (ProductIsOnSale[indexPath.row] == 1) {
             ...
             collectionCell.collectionPrice.attributedText = attributedText
             
             collectionCell.collectionSalePrice.text = String(Int(ProductSalePrice[indexPath.row])) + " RC"
             collectionCell.collectionSalePrice.isHidden = false
         } else {
             collectionCell.collectionPrice.text = String(Int(ProductPrice[indexPath.row])) + " RC"
             collectionCell.collectionSalePrice.isHidden = true
         }
            ...
            for user in allUser as! [Users] {
                if user.username == username {
                    if ((user.wishlistItems?.contains(Int(ProductID[indexPath.row])))!){
                        collectionCell.wishlistBtn.setBackgroundImage(UIImage(systemName:"heart.fill"), for: .normal)
                    } else {
                        collectionCell.wishlistBtn.setBackgroundImage(UIImage(systemName:"heart"), for: .normal)
                    }
                }
            }

        } catch {print(error)}
        collectionCell.wishlistBtn.tag = Int(ProductID[indexPath.row])
        collectionCell.wishlistBtn.addTarget(self, action: #selector(self.addToWishlist), for: .touchUpInside)
    ...
    override func viewDidLoad() {
        ...
        getProducts(type: display, sortkey: sortkey, ascend: ascend)
        animate()
    }
    ...
    func getProducts(type: String, sortkey: String, ascend: Bool){
